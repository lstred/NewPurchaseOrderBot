...
|---|---|---|---|
| **AI v5.1 — Drop Top Concerns, lead with Recommended Actions, fix clearance CC misattribution, spiky-month tagging, Featured Cost Center daily rotation** | `app/ai/brief.py`, `app/ai/schema.py`, `app/data/store.py` | After v5.0 shipped, the user reviewed the rendered brief (CC 010 — Residential Carpet) and reported five things in one message: (a) **clearance items appearing under the wrong CC** (FUS06P / FUS5053P showed up under CC 010 even though both SKUs live in CC 050) — diagnosis confirmed the per-CC `aging_clearance` table for CC 010 is correctly empty (CC 010 has only 1 aged item, below the 2-row threshold), so the AI was hallucinating clearance bullets by reading from the cross-portfolio AGING INVENTORY table at the bottom of the prompt and pasting the worst-aged SKUs into a CC where they don't belong; (b) *"remove the Top Conerns section and move Recommended Actions (top 5) to the top where top conerns was"*; (c) *"some items have a month or two that can sku the numbers like this one QSTBAXTCROFTL ... when you see skus that have a particular month that is more than triple the average month then should somehow be noted"* (verified — QSTBAXTCROFTL had a 77,801 SY February 2026 spike against a 12,470 SY mean, 6.2× ratio); (d) *"new section called featured cost center ... new each day until it goes through all of them and then goes back to the top again ... insights regarding sales trends ... by product line or by price class ... atleast three basic insights basically the good, the bad and the ugly"*; (e) general orphan/optimization pass plus CLAUDE.md/push/restart cycle. | **(1) Clearance CC misattribution fix.** Removed the cross-portfolio `### AGING INVENTORY` table from `build_brief_prompt()` entirely — it was the source of the AI's hallucination supply because clearance was redundant with the per-CC `aging_clearance` tables anyway. Tightened the schema with an explicit STRICT v5.1 rule in BOTH the per-CC prompt block and the schema's Cost Center Breakdown section: *"a `[CLEARANCE]` bullet may ONLY appear in the CC section that explicitly lists the SKU in its `aging_clearance` table. NEVER copy clearance items between CCs, never invent clearance bullets from the cross-portfolio tables, and never add a clearance row to a CC whose breakdown has no `aging_clearance` table at all."* The cross-portfolio supporting-tables block was relabelled *"Reference data only — DO NOT render bullets directly from these tables. Every actionable bullet must come from the per-CC breakdown above so items always land under the correct cost center."* **(2) Section reorder — Top Concerns gone, Recommended Actions leads.** `build_brief_prompt()` no longer emits the `## TOP CONCERNS` section (the data block + the AI-output section); the v5.0 explanatory `## TOP CONCERNS (v5.0 — only buyer-actionable items)` 17-line block was replaced by the v5.1 `## SPIKY-SKU MARKER` block (which is shorter and explains the new tag). The internal `data.top_concerns` DataFrame is still computed because it powers `_annotate_new_items` (the NEW-marker SKU set is the union of top_concerns + every CC table). `schema.py` system prompt restructured: `## Top Concerns` deleted (was a 16-line section), `## Recommended Actions (top 5)` was moved up to slot directly after `# Executive Summary` so the buyer's playbook is the first thing they read; the section text was extended with *"v5.1 lifts this list to the TOP of the brief (immediately after the executive summary) so the buyer sees the day's playbook first."* **(3) Spiky-month detection.** New constants `_SPIKY_MIN_MONTHS = 6`, `_SPIKY_MIN_MAX_MONTH_QTY = 50.0`, `_SPIKY_RATIO = 3.0`, `_SPIKY_LOOKBACK_DAYS = 365`. New helper `_compute_spiky_skus(bundle, target_date) -> dict` builds monthly buckets from `bundle.orders` (last 365 days, non-backorder, qty > 0), groups by `(base_sku, year-month)`, then groups by base_sku to compute `(months, max_qty, mean_qty)`; flags SKUs where `months >= 6 AND max_qty >= 50 SY AND max_qty > 3*mean_qty`. Returns `{base_sku: {spike_month, spike_qty, mean_qty, ratio, months}}`. New helper `_annotate_spiky(data)` attaches `is_spiky` / `spike_month` / `spike_ratio` columns to `needs_reorder_no_po`, `excessive_incoming_pos`, `massive_overstock`, `redflag_new_pos` (every actionable table whose action depends on avg-daily volume). Both helpers are called in `gather_brief_data` immediately after `_build_actionable_metrics`. New `BriefData.spiky_skus: dict` field stores the raw map. Schema instructs the AI to append `[SPIKY: <month> <ratio>x avg]` at the END of every bullet (after rationale, after `[NEW]` modifier) for any row with `is_spiky=True`. **(4) Featured Cost Center daily rotation.** New persistence file `_FEATURED_CC_FILE = APPDATA_DIR / "featured_cc_rotation.json"` in `store.py` with helper `get_next_featured_cc(target_date, all_ccs_sorted) -> str`: idempotent for same-day re-runs (returns the same CC if `state.last_date == target_iso`), advances one slot in the sorted CC list and wraps around for new days, returns `""` if no CCs available. State shape: `{ "last_date": "YYYY-MM-DD", "last_cc": "010" }`. New `BriefData.featured_cost_center: dict` field. New helper `_build_featured_cost_center(data, sm, bundle, target_date)` selects the rotation pick, then assembles: KPIs (skus, inventory_sy, on_order_sy, median DOI, stockouts, overstock, aged_365), trend tables `by_product_line` and `by_price_class` (pre-filtered with sample-size guard `lines_365d >= 30 OR qty_365d >= 500 SY` to kill flukes; columns `qty_90d`, `qty_180d`, `qty_365d`, `qty_prior_90d`, `lines_365d`, `trend_pct` (= prior-90d vs latest-90d %), `other_ccs_qty_365d`, `share_of_total`, friendly `label`), `top_growers_90d` and `top_decliners_90d` (combined view across both dimensions, ranked by `trend_pct`, with secondary guard `qty_prior_90d >= 100 OR qty_90d >= 100`), `spiky_skus_in_cc` (top 10 by ratio for SKUs that live in this CC AND appear in `data.spiky_skus`), and `comparatives` dict (`cc_qty_365d`, `portfolio_qty_365d`, `cc_share_pct`, `cc_lines_365d`). `build_brief_prompt()` emits a new `## FEATURED COST CENTER OF THE DAY` section with the rotation explainer + KPI line + comparatives line + every table prefixed `featured.<name>`. `schema.py` instructs the AI to write a `## Featured Cost Center — CC <code> — <name>` heading followed by EXACTLY THREE labelled bullets (**The Good** / **The Bad** / **The Ugly**), each backed by a number from the supplied tables and ideally a comparative (vs prior 90d or vs portfolio share). **(5) Smoke test under `-W error::FutureWarning`** (May 12 2026 dataset, fresh `featured_cc_rotation.json`): 728 spiky SKUs detected; **QSTBAXTCROFTL flagged** with `spike_month=2026-02 spike_qty=77,801 mean_qty=12,470 ratio=6.24 months=10` (matches the user's example exactly); 14 CCs in breakdown with aging_clearance attached to 6 (013, 027, 031, 050, 051, 052) — **CC 010 correctly has NO aging_clearance** (only 1 aged item, below threshold), so the bug source is gone; Featured CC = `010 — CARPET RESIDENTIAL` with all 5 expected tables (`by_product_line`, `by_price_class`, `top_growers_90d`, `top_decliners_90d`, `spiky_skus_in_cc`); KPIs: 850 SKUs, 384,106 SY inv, 293,387 SY OO, median DOI 95d, 6 stockouts, 593 overstock, 66 aged-≥365d; comparatives: 1.21M SY 365d / 23,797 lines / 3.9% portfolio share against 31.2M SY total; prompt size **77 KB** (down from 82 KB v5.0); `'TOP CONCERNS' in prompt` is False; `'FEATURED COST CENTER' in prompt` is True; cross-portfolio AGING INVENTORY block confirmed absent; zero FutureWarnings. |
| **AI v5.0 — Lead-time-aware DOI threshold + always-show 2 clearance per CC + orphan cleanup** | `app/ai/brief.py`, `app/ai/schema.py` | After v4.9 shipped, the user reviewed the brief and asked for two refinements: (a) *"if lead time over 99 then stick with 700 doi as a threshold to show overstock risk if its under that number then use 365 doi as the threshold"* — fast-turn SKUs with short lead times can self-correct inside one re-buy cycle, so the 700d floor was hiding genuinely actionable inbound on quick-turn items; (b) *"would like you to choose up to two clearance items if there are atleast two to show in each cost center section"* — clearance-as-fallback (v4.8/4.9) meant most CCs never showed any aged-stock context, but the buyer wants a steady 2-item markdown shortlist visible alongside the actionable rows. Plus a general orphan/optimization pass and CLAUDE.md/restart cycle. | **(1) Lead-time-aware overstock floor.** New constants in `brief.py`: `_INCOMING_OVERSTOCK_DOI_DAYS_SLOW_LT = 700`, `_INCOMING_OVERSTOCK_DOI_DAYS_FAST_LT = 365`, `_LEAD_TIME_FAST_THRESHOLD_DAYS = 99`. New helper `_overstock_doi_threshold(sm) -> pd.Series` returns a per-row threshold using `numpy.where(lt <= 99, 365, 700)` (`from numpy import where as np_where` added at module top). Helper applied at every overstock-detection site: `excessive_incoming_pos` filter, `massive_overstock` global filter (`multi_year_cover = doi_proj >= _overstock_doi_threshold(mo)`), `redflag_new_pos` filter (uses `data.yesterday_new_pos`'s already-enriched `lead_time_days`), and the per-CC `massive_overstock` overlay. The legacy `_INCOMING_OVERSTOCK_DOI_DAYS = 700` alias was removed entirely after a workspace grep confirmed zero remaining references. **(2) Always-show 2 clearance per CC.** `_build_cost_center_breakdown()` rewritten so `aging_clearance` is no longer a fallback gated by `if not slot["buckets"]`; it's now attached to EVERY CC where `len(per_cc_aging) >= _CLEARANCE_PER_CC` (2). The clearance table is `sub.head(2).reset_index(drop=True)` so the renderer always shows exactly 2 rows when present. CCs with fewer than 2 aged items skip the table to avoid one-off noise. The `incoming/reorder/clearance` bucket bookkeeping is still tracked internally and stripped before return. **(3) Orphan-code cleanup pass.** Removed five fully-dead computation blocks from `_build_actionable_metrics` whose outputs were already being zeroed at the bottom of `gather_brief_data` (decelerating_velocity, pos_arriving_after_stockout, dead_stock, receipts_pushed_to_overstock, oversized_pos_vs_demand). Removed three more dead blocks in `gather_brief_data` itself (active_stockouts, runout_risk, overstock_with_open_po). Removed the entire `_empty = pd.DataFrame()` zero-out block (no longer needed — the fields are gone). Pruned `BriefData` dataclass: deleted 8 unused fields (`active_stockouts`, `runout_risk`, `overstock_with_open_po`, `decelerating_velocity`, `pos_arriving_after_stockout`, `dead_stock`, `receipts_pushed_to_overstock`, `oversized_pos_vs_demand`). Verified via workspace grep that none of the removed fields are referenced anywhere outside `brief.py` (the renderer, schema, store, and UI never touched them). The portfolio-snapshot KPI line (`Active stockouts: ... | Runout risk: ...`) was preserved because it reads `sm["stockout_flag"]` / `sm["runout_risk"]` columns directly, not the removed dataclass fields. **(4) Prompt + comment refresh.** `_CONCERN_SEVERITY` comment block rewritten for v5.0 ("Threshold for `incoming` is now lead-time-aware: 700d when lead_time_days > 99, else 365d"). Top-concerns prompt copy updated: *"pushing days-of-inventory past its overstock floor (700d for slow-LT items > 99-day lead time, 365d for fast-LT items)"*. Per-CC prompt copy rewritten to reflect always-show-2 clearance: *"every CC with at least 2 aged-inventory items also receives a 2-row aging_clearance table. Render those as `[CLEARANCE]` bullets at the BOTTOM of the CC's section, after the actionable rows. NEVER add `[NEW]` badges to clearance rows."* Cross-portfolio supporting-table headings updated to *"push DOI past lead-time-aware floor"*. `_emit()` action_hint strings genericised from *"past 700 days"* to *"past its overstock floor"* so the AI doesn't anchor on a single number. **(5) `schema.py` system prompt rewritten for v5.0**: the OVERSTOCK RISK definition explicitly states the 700d-slow/365d-fast split tied to `lead_time_days > 99`; CLEARANCE is now an active third category (no longer "fallback ONLY") whose rows render at the BOTTOM of each CC alongside actionable rows; DO-NOT list now reads *"if a row's DOI is below 700 (slow-LT) or 365 (fast-LT), do not call it overstock risk"*. The "If a CC's only table is `aging_clearance`" branch is preserved for the few CCs with no actionable rows. **(6) Smoke test under `-W error::FutureWarning`** (May 12 2026 dataset): top_concerns 66 rows across 2 concern types (`incoming_overstock`, `needs_reorder`); slow-LT bucket 8 rows with min DOI **703.1** (correctly above 700); fast-LT bucket 38 rows with min DOI **370.0** (correctly above 365); 29 of those 38 fast-LT rows are in the new 365–700 DOI band that v4.9 would have hidden — direct confirmation the lead-time gate is working; CCs total 14 of which 6 have an `aging_clearance` table with exactly 2 rows each; prompt size **82 KB** (down from 128 KB v4.9 / 144 KB v4.8 / 288 KB v4.7); zero FutureWarnings; renderer pills (`OVERSTOCK RISK → act-overstock`, `REORDER → act-reorder`, `CLEARANCE → act-clearance`, `NEW → act-new`) unchanged and rendering correctly. |
